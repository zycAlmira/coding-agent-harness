...
## 2026-08-10 根因修复:缓存命中绕过停机判断致无限循环(系统性排查)

- 触发:用户反馈「agent 还是终止了,不能再重复犯这种问题」——要求系统性找根因,不再打补丁。
- 系统性排查(对比多轮历史):发现最新历史 steps=18 但 outcome=max_rounds——18 步不该触发 60 硬上限。实测复现:**缓存命中分支 return None 但主循环每次仍调 LLM,且绕过 decide_stop**——agent 固执请求读同一已缓存文件时,steps 不增但 rounds 无限增长,每次 LLM 网络请求,直到 hard_max_rounds 才停。这是上轮加缓存时引入的回归,也是「18 步 max_rounds」的真根因。
- 修复:
  1. 缓存命中分支加 decide_stop 检查(轮数仍增长必须停机判断)
  2. 连续 3 次命中缓存 → 直接 stuck 停机(死循环,继续问 LLM 是空转)
  3. 命中时写历史消息告知「已缓存」,让 agent 有依据继续
- 端到端:固执 LLM 连续请求读同一文件 → 4 次 LLM 调用即 stuck 停机(原无限循环 60 轮,2 分钟超时)。
- 验证:145 passed,lint 全过。
- 教训 36:**"不能再犯"要求系统性根因分析,而非逐个补丁**——对比多轮历史发现 steps≠rounds 的不一致,才定位到缓存分支绕过停机判断;每轮修复后必须验证"最坏情况是否被兜底",否则回归悄无声息。
- 教训 37:**新增的优化(缓存/批处理)必须保持既有停机语义**——缓存命中是新路径,忘了在其上做 decide_stop,导致绕过安全阀;任何新增执行路径都要过一遍"停机判断是否覆盖"。

## 2026-08-10 意外终止根因修复:缓存命中计数跨任务累积

- 触发:用户反馈「agent 意外终止」。
- 真实历史(dc5b3ad97d9d,13 步 stuck)分析:动作全是不同文件,无连续同文件重复读,却 stuck 停机——只有缓存连续命中 3 次触发 stuck,但动作无重复,矛盾。
- 根因:`_cache_hits` 用 getattr 惰性初始化,未在 run() 重置。WebUI 续聊(_active 复用同一 loop 实例)时,上一任务的缓存命中数累积到下一任务——某任务命中 1-2 次 + 前任务命中,累计 3 即误触发 stuck 停机。
- 修复:run() 任务开始时重置 _cache_hits = 0。
- 验证:148 passed(新增同实例跨任务重置测试),lint 全过。
- 教训 38:**状态必须显式初始化,惰性 getattr 是隐藏累积的温床**——_cache_hits 本应像 _read_cache 一样在 run() 重置;跨任务复用实例(WebUI 续聊)让"隐式全局计数"变成误判来源。**每个状态变量都要自问:它跨任务应该重置吗?**
- 教训 39:**"意外终止"先看有没有"矛盾信号"**——13 步全不同文件却 stuck,与触发条件矛盾,矛盾处就是 bug 所在;顺着矛盾找状态泄漏,比猜 agent 行为快得多。

## 2026-08-10 缓存重构:从"拒绝需求"到"满足需求"(用户批评驱动)

- 触发:用户反馈「agent 陷入重复读取,自动停止。这样做解决不了问题」——停机止损让任务失败,治标不治本。
- 根因洞察(关键转折):agent 反复读取是因为**读不到完整内容(截断)又不敢在不完整信息下动手**——它要"看到完整文件",不是"被告知别读了"。之前缓存是拒绝需求(提示+stuck 停机),任务失败。**正确做法是满足需求:已读内容累积,重复读时回灌完整拼接。**
- 重构:
  1. 缓存 {text, total, read_rows, full}:整读未截断→full;分段累积 read_rows 覆盖全部→full
  2. 完整命中回灌缓存全文(agent 真正看到),非跳过
  3. 分段读累积,多段覆盖后重复读回灌全部段拼接
  4. **不再 stuck 停机**(hard_max_rounds 兜底真死循环);3 次命中仅提示 search_file
  5. 修复首次读重复执行 bug
- 端到端:大文件截断→分段读→full→重复读回灌完整→写代码完成。
- 验证:150 passed,lint 全过。
- 教训 40:**用户批评"解决不了问题"是设计方向的信号**——停机止损只是防御,任务没完成就是失败;agent 重复读的深层需求是"看到完整内容",给内容(满足)比拦行为(拒绝)有效得多。
- 教训 41:**LLM 的"确认偏好"要用"信息供给"化解,不是"行为限制"**——它反复读是信息不足下的自我保护;累积拼接回灌让信息只增不减,确认偏好自然消退,无需停机干预。

## 2026-08-10 修复"接近完成却 stuck 误杀"(no_change 语义)

- 触发:用户反馈「agent 只运行 13 轮就终止了,这次很接近完成任务」。
- 真实历史(13 轮 stuck)分析:agent 表现完美——读 8 文件→WriteFile 完成 6 处填空→RunTests 验证。但第 10、12 步 RunTests 连续失败(同一测试文件)→ no_change_streak=2 → stuck。第 11 步 ListDir(找 pom.xml)是 agent 在修复,却被"无变化"误判。
- 根因:no_change_streak 只追踪测试失败集合,不区分"agent 是否在推进"。写完代码跑测试失败 2 次就被判"无变化"——实际 agent 正在修复。
- 修复:非 RunTests 动作(WriteFile/ListDir/SearchFile 探索与修复)重置 no_change_streak——agent 在推进就不算无变化;测试连续失败仅在无任何中间动作时触发 stuck。
