...
- [x] Task 22: CI 配置 — complete (commit 待提交, review 待派)。`.gitlab-ci.yml`:stages=[test,build];`unit-test` job(§五.6 硬要求,`uv sync --frozen`+`uv run pytest -q`,与 `make test` 一致,MR+分支触发);`build-image` job(docker-in-docker,仅 main+exists Dockerfile,Task 23 就绪后自动生效)。`test_ci_config.py` 4 确定性单测(PyYAML 解析,§A.4 精神:配置也可验证)。顺带闭合 Task 1 Minor(ruff 进 dev deps,`make lint` 35→0 全绿)+ models.py 审查 Minor(Any 移顶、F821 前向引用用 TYPE_CHECKING)。82 passed。取舍:§4.8 提 GitHub Actions 但 §五.6 硬要求 gitlab-ci.yml,以后者为准;不强跑 Windows 矩阵(mock 测不暴露 Task 7 shlex 弱点,登记 follow-up)。
