...
这种放大器效应在后期的真实修复中同样成立:例如"探索/修复动作应重置 no_change_streak"这条新语义,我先写了一个让测试红掉的用例(两次测试失败之间插入 ListDir,断言不 stuck),再改 `_process_action` 重置计数——红测试精确指出了"停机机制不理解 agent 进展"这个缺陷,修复后测试变绿,语义才真正钉死。
...
**凭据(§3.1)** 迫使想清:① "不进 git(含 history)"意味着连 `export` 都不能有,`.env` 须经环境加载而非 shell history;② "查看不回显明文"意味着 status 端点只返回布尔语义,不是"显示部分字符";③ 录入用 `getpass` 隐藏输入、测试用 in-memory keyring 后端,才既不碰真实 Keychain 又覆盖存取逻辑;

**分发(§3.2)** 迫使想清:① 容器默认 mock 模式才能满足 §五.9"可访问 WebUI"又不碰 key;真实 LLM 经 WebUI 凭据端点录入而非镜像预置;② 部署脚本(阿里云 `deploy-aliyun.sh`)把"构建→上传→启动"封装成一条命令,key 只在目标机经端点录入,镜像里没有任何凭据——这正是"别人如何在全新机器上安全跑起来"的答案。
